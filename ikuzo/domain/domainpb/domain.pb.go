...
type Entry struct {
	...
	// SortValue is an alternate key for facet ordering. When set, facet
	// aggregations may order buckets by min(sortValue) while keeping
	// Value as the visible bucket key — e.g. an agent whose Value renders
	// as "Willem Elsschot" (first-name first) can carry SortValue
	// "Elsschot, Willem" so the exploded facet lists by surname without
	// changing what the user sees.
	SortValue     string `protobuf:"bytes,18,opt,name=sortValue,proto3" json:"sortValue,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *Entry) GetSortValue() string {
	if x != nil {
		return x.SortValue
	}
	return ""
...
const file_ikuzo_domain_domainpb_domain_proto_rawDesc = "" +
	...
	"\x05value\x18\x02 \x01(\v2\x0f.domainpb.EntryR\x05value:\x028\x01\"\x91\x04\n" +
	"\x05Entry\x12\x0e\n" +
	"\x02ID\x18\x01 \x01(\tR\x02ID\x12\x14\n" +
	...
	"\x06Inline\x18\x11 \x01(\v2\x12.domainpb.ResourceR\x06Inline\x12\x1c\n" +
	"\tsortValue\x18\x12 \x01(\tR\tsortValue\"0\n" +
	"\n" +
	"IndexRange\x12\x10\n" +
