...
type rdfXMLDecoder struct {
	...
	seed        string
	idGenerator BlankNodeIDGenerator
}

// newRDFXMLDecoder creates a new decoder with optional configuration
func newRDFXMLDecoder(r io.Reader, opts ...Option) *rdfXMLDecoder {
	d := &rdfXMLDecoder{
		dec:       xml.NewDecoder(r),
		nextState: parseXMLTopElem,
		seed:      "default", // sensible default
	}

	// Apply default options
	d.idGenerator = &IncrementalIDGenerator{counter: 0}
...
func NewRDFXMLDecoder(r io.Reader, opts ...Option) *rdfXMLDecoder {
	return newRDFXMLDecoder(r, opts...)
}
...
// appendElementToPath adds an XML element to the current path.
// If the element has a namespace, it's combined with the local name.
// For elements without a namespace, only the local name is added.
// This ensures consistent path construction for both namespaced and non-namespaced XML.
func (d *rdfXMLDecoder) appendElementToPath(elem xml.StartElement) {
	// Construct path element with namespace and local name
	pathElement := elem.Name.Space + elem.Name.Local

	// Add unique identifiers from attributes if present
	if about := attrRDF(elem, elAbout); about != nil {
		pathElement += ":about=" + about[0].Value
	}
	if resource := attrRDF(elem, elResource); resource != nil {
		pathElement += ":resource=" + resource[0].Value
	}
	if id := attrRDF(elem, elID); id != nil {
		pathElement += ":id=" + id[0].Value
	}
	if nodeID := attrRDF(elem, elNodeID); nodeID != nil {
		pathElement += ":nodeID=" + nodeID[0].Value
	}

	// Include position information
	pathElement += fmt.Sprintf(":pos=%d", len(d.ctx.Path))

	d.ctx.Path = append(d.ctx.Path, pathElement)
	slog.Debug("append to path",
		"element", pathElement,
		"full_path", d.ctx.Path,
		"depth", len(d.ctx.Path))
}

// removeLastElementFromPath removes the last element from the current path
// when leaving an XML element. This maintains the path stack in sync with
// XML document traversal.
func (d *rdfXMLDecoder) removeLastElementFromPath() {
	if len(d.ctx.Path) > 0 {
		removed := d.ctx.Path[len(d.ctx.Path)-1]
		d.ctx.Path = d.ctx.Path[:len(d.ctx.Path)-1]
		slog.Debug("remove from path",
			"removed", removed,
			"remaining_path", d.ctx.Path,
			"remaining_depth", len(d.ctx.Path))
	}
}

// appendRDFTermToPath adds an RDF term to the current path.
// This is used for special RDF constructs like Collection that aren't
// represented as XML elements but need to be part of the path.
func (d *rdfXMLDecoder) appendRDFTermToPath(term string) {
	pathElement := rdfNS + term + fmt.Sprintf(":pos=%d", len(d.ctx.Path))
	d.ctx.Path = append(d.ctx.Path, pathElement)
	slog.Debug("append RDF term to path",
		"term", term,
		"element", pathElement,
		"full_path", d.ctx.Path,
		"depth", len(d.ctx.Path))
...
func parseXMLNodeElem(d *rdfXMLDecoder) parseXMLFn {
		...
		if elem.Name.Space == rdfNS {
			...
			case elDescription:
				...
				if as := attrRDF(elem, elNodeID); as != nil {
					...
					if as[0].Value == "" {
						// Generate deterministic ID for empty nodeID
				...
				if len(elem.Attr) == 0 || d.current.Subj == nil {
					// For blank nodes with no ID, use path-based generation
					d.current.Subj = Blank{id: d.GenerateID()}
				}
	...
	case xml.EndElement:
		// Remove last element from path when leaving element
...
func parseXMLColl(d *rdfXMLDecoder) parseXMLFn {
	// Add collection element to path
	d.appendRDFTermToPath("Collection")

	// Create first blank node for collection
	d.current.Obj = Blank{id: d.GenerateID()}
	d.triples = append(d.triples, d.current)
...
outer:
	for {
		...
		case xml.StartElement:
			...
			if elem.Name.Space == rdfNS && elem.Name.Local == elDescription {
				if a := attrRDF(elem, elAbout); a != nil {
					if first {
						...
						// Add "Next" to the path via context
						d.appendRDFTermToPath("Next")
						d.current.Obj = Blank{id: d.GenerateID()}
						// Remove "Next" from path since we're done with it
						d.removeLastElementFromPath()
			...
			// Remove element from path after processing
			d.removeLastElementFromPath()
		...
		if a.Name.Space == rdfNS {
			...
			case lname:
				if lname == elNodeID || lname == elID {
					if a.Value != "" && !rgxpNCName.MatchString(a.Value) {
...
// GenerateID generates a blank node identifier using the configured ID generation strategy.
// The returned identifier will be deterministic or sequential depending on the decoder's
// configuration. The generated ID will always start with "_:b" followed by the
// strategy-specific identifier.
func (d *rdfXMLDecoder) GenerateID() string {
